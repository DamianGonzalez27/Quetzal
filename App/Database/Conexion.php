...
class Conexion
{
    ...
    public static function openConexion()
    {
        if(!isset(self::$conexion))
        {
            try
            {
                self::$conexion = new PDO('mysql:host='.CONFIGS['database']['server'].'; dbname='.CONFIGS['database']['name'].'; port='.CONFIGS['database']['port'], CONFIGS['database']['user'], CONFIGS['database']['pass']);
